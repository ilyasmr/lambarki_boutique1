                ...
                filteredInvoices.slice().reverse().map((invoice) => (
                  <tr key={invoice.id} className="block md:table-row text-xs hover:bg-gray-50 transition p-4 md:p-0 bg-white rounded-2xl shadow-sm border border-gray-100 md:border-none md:shadow-none md:rounded-none md:bg-transparent relative">
                    ...
                    <td className="flex justify-between md:table-cell py-2 md:py-3.5 md:px-3 text-gray-900 font-bold border-t border-dashed border-gray-100 md:border-none mt-3 md:mt-0 pt-3 md:pt-3.5">
                      ...
                      <span className="text-sm md:text-xs">{invoice.clientName?.replace(/\(?صندوق\)?/g, '').trim()}</span>
                    </td>
            ...
            <div className="flex items-center gap-3 border-b border-gray-50 pb-3">
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm mb-6 bg-rose-50 p-4 rounded-xl border border-rose-100/50 leading-relaxed text-center">
                  ...
                  <span className="text-slate-800 font-extrabold">{invoiceToDelete.clientName?.replace(/\(?صندوق\)?/g, '').trim()}</span> ?
                </p>
              </div>
            </div>
