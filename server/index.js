...
async function getWithdrawals(req, res) {
  try {
    const result = await pool.query('SELECT * FROM withdrawals ORDER BY date DESC');
...
async function getDrawerState(req, res) {
  try {
    let result = await pool.query("SELECT * FROM drawer_state WHERE id='singleton'");
    if (result.rows.length === 0) {
      await pool.query("INSERT INTO drawer_state (id) VALUES ('singleton')");
      result = await pool.query("SELECT * FROM drawer_state WHERE id='singleton'");
    }
...
app.post('/api/drawer_state/clear', async (req, res) => {
  try {
    ...
    await pool.query("UPDATE drawer_state SET withdrawals_adjustment=0, cash_income_adjustment=0, drawer_balance_adjustment=0 WHERE id='singleton'");
    res.json({ status: 'ok' });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
